...
function startBossBattle(manual=false){
  ...
  if(manual&&player?.alive&&player.level>=10&&eligible.length<2){
    ...
    if(sparring){while(sparring.level<10){sparring.level++;autoStat(sparring);if(EVOLUTIONS[sparring.level])autoEvolve(sparring,sparring.level);}eligible=fighters.filter(f=>f.alive&&f.level>=10);}
  }
...
function startGame(){
  ...
  for(let i=0;i<15;i++){const f=new Fighter(BOT_NAMES[i%BOT_NAMES.length],rand(120,WORLD.w-120),rand(120,WORLD.h-120));const levels=randi(0,4);for(let l=0;l<levels;l++){f.level++;autoStat(f);}fighters.push(f);}
  for(let i=0;i<28;i++)monsters.push(new Monster(rand(80,WORLD.w-80),rand(80,WORLD.h-80)));
  if($('boostToggle').checked&&meta.boosts>0){meta.boosts--;for(let i=1;i<5;i++){player.level++;autoStat(player);}saveMeta();}
